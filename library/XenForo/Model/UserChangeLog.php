...
class XenForo_Model_UserChangeLog extends XenForo_Model
{
	...
	public function getChangeLogs(array $conditions = array(), array $fetchOptions = array())
	{
		...
		if (!empty($logs))
		{
			...
			{
				if (!isset($groupedLogs[$log['edit_date']]))
				{
					$groupedLogs[$log['edit_date']] = array(
						'user' => array(
							'user_id' => $log['user_id'],
				...
				$groupedLogs[$log['edit_date']]['fields'][$logId] = $this->prepareField($log);
			}
		}
	...
	public function countChangeLogs(array $conditions)
	{
		...
		return $this->_getDb()->fetchOne("
			SELECT COUNT(DISTINCT CONCAT(logs.user_id, '-', logs.edit_date))
			FROM xf_user_change_log AS logs
			WHERE " . $whereClause
		);
	}
