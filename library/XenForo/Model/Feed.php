...
class XenForo_Model_Feed extends XenForo_Model
{
	...
	public function getFeeds(array $conditions = array(), array $fetchOptions = array())
	{
		...
		switch (!empty($fetchOptions['order']) ? $fetchOptions['order'] : '')
		{
			case 'last_fetch':
			case 'title':
				$orderBy = 'feed.' . $fetchOptions['order'];
				break;
	...
	public function prepareFeedConditions(array $conditions, array &$fetchOptions)
	{
		...
		if (!empty($conditions['time_now']))
		{
			$sqlConditions[] = "feed.last_fetch + feed.frequency < " . intval($conditions['time_now']);
		}
