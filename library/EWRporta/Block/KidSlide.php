...
class EWRporta_Block_KidSlide extends XenForo_Model
{
	public function getModule(&$options, $page, $category = false)
	{
		...
		$knew = $this->_getDb()->fetchAll("
			...
			WHERE (xf_thread.node_id IN (".$this->_getDb()->quote($options['forum']).") OR EWRporta_promotes.promote_date < ?)
				...
				AND (? - xf_thread.post_date) < ?*86400
				AND IF(EWRporta_promotes.promote_date IS NULL, xf_thread.post_date, EWRporta_promotes.promote_date) < ?
			ORDER BY $fromOrder view_count DESC, promote_date DESC
			LIMIT ?, ?
		", array(XenForo_Application::$time,XenForo_Application::$time, $options['cutoff'],XenForo_Application::$time, $page, $options['limit']));
		
		foreach ($knew AS &$post)
		{
			...
			list($post['month'], $post['day'], $post['year'])  = explode('.', $strtime->format('n.d.y'));
