...
class EWRporta_Block_FrontPage extends XenForo_Model
{
	public function getModule(&$options, $page, $category = false)
	{
		...
		{
			//Thumb cho bài viết, by Kid
		$attachmentModel = new XenForo_Model_Attachment;
		
		$string = $post['message'];	
		$post['kid_thumb'] = array();		
		...
		//$attachments = $attachmentModel->getAttachmentsByContentId('post', $post['first_post_id']);
		//foreach ($attachments AS $attachment)
		//{
		//	if($attachment['width']){
		//		$attachmented= $attachmentModel->prepareAttachment($attachment);
		//		$post['kid_thumb'][] = '<img src="'.$attachmented['thumbnailUrl'].'" class="img_thumb" alt="'.$post["title"].'"/>';
		//		
		//	}
		//}
		...
		{
			if(isset($matches2)){	 
			$post['kid_thumb'][] = '<img src="timthumb.php?src='.$matches2.'&h=110&w=0&zc=1" class="img_thumb" alt="'.$post["title"].'"/>';
			}
		}
		...
		{
			if(isset($matchesv2)){	 
			$post['kid_thumb'][] = '<img class="play_icon" alt="play icon" src="styles/default/icon/play_icon.png"><img src="http://i1.ytimg.com/vi/'.$matchesv2.'/mqdefault.jpg" class="img_thumb" alt="'.$post["title"].'"/>';			
			}
		}
		
		$vowels = array('[IMG]','[/IMG]','[media=youtube]','[/media]');
		
		$post['kid_thumb'] = str_replace($vowels, "", $post['kid_thumb']);	
		//Hết thumb
