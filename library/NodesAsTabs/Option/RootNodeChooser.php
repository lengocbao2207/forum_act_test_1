...
// DUPLICATE OF XenForo_Option_NodeChooser
// EXCEPT FOR ADDED SECTION IN getNodeOptions()
abstract class NodesAsTabs_Option_RootNodeChooser
{
	/**
	...
	public static function getNodeOptions($selectedForum, $includeRoot = false, $filter = false)
	{
		...
		/* ADDED FOR NODESASTABS */
		$optionsModel = XenForo_Model::create('NodesAsTabs_Model_Options');
		$nodeTabs = $optionsModel->getEligibleRootNodes();
		foreach ($options AS &$option)
		{
			if (!empty($option['value']) && !isset($nodeTabs[$option['value']]))
			{
